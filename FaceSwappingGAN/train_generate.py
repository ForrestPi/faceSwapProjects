...
if __name__ == '__main__':
	...
	generator = model.Generator(model.Globalgenerator(2, 2, 3, 3, 71, 3), resume_path='./vggface.pth', consistency_iter=args.con_iter)
	...
	for batch_idx, (inputs, target_hmap, target, target_seg, m_hmaps) in enumerate(trainloader):
		...
		output, loss_gen, con_out = generator(inputs, target, target_seg, target_hmap, m_hmaps)
		...
		if (true_idx % 500 == 0) and (batch_idx > 5000):
			for i in range(10):
				cv2.imwrite('work/vis/%d_%d_inputs.jpg'%(true_idx, i), (inputs.cpu().detach().numpy()[i, 0:3, :, :].transpose(1,2,0) + 1)* 127.5)
				cv2.imwrite('work/vis/%d_%d_target.jpg'%(true_idx, i), (target.cpu().detach().numpy()[i, :, :, :].transpose(1,2,0) + 1) * 127.5)
				cv2.imwrite('work/vis/%d_%d_output.jpg'%(true_idx, i), (output.cpu().detach().numpy()[i, :, :, :].transpose(1,2,0) + 1) * 127.5)
				cv2.imwrite('work/vis/%d_%d_conout.jpg'%(true_idx, i), (con_out.cpu().detach().numpy()[i, :, :, :].transpose(1,2,0) + 1) * 127.5)
		...
		if (batch_idx % 100 == 0):
			writer.add_images('target', (target[0:4, [2, 1, 0], :, :]+1)/2, 0)
			writer.add_images('output', (output[0:4, [2, 1, 0], :, :]+1)/2, 0)
