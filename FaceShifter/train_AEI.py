...
for epoch in range(0, max_epoch):
    ...
    for iteration, data in enumerate(dataloader):
        ...
        if iteration % show_step == 0:
            ...
            cv2.imwrite('./gen_images/{}.jpg'.format(iteration), image.transpose([1,2,0]))
        print(f'epoch: {epoch}    {iteration} / {len(dataloader)}')
        print(f'lossD: {lossD.item()}    lossG: {lossG.item()} batch_time: {batch_time}s')
        print(f'L_adv: {L_adv.item()} L_id: {L_id.item()} L_attr: {L_attr.item()} L_rec: {L_rec.item()}')
        if iteration % 1000 == 0:
            torch.save(G.state_dict(), './saved_models/G_{}.pth'.format(iteration))
            torch.save(D.state_dict(), './saved_models/D_{}.pth'.format(iteration))
